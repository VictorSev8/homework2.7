...
public class EmployeeBook {
    ...
    public void addEmployee(String name, int salary, int department) {
        addEmployee(new Employee(name, salary, department));
    }
    ...
    public void printEmployeesWithSalaryLessThan(int bound) {
        System.out.println("Сотрудники с ЗП меньшей, чем " + bound + ":");
        for (Employee employee : employees.values()) {
            if (Objects.nonNull(employee) && employee.getSalary() < bound) {
                System.out.println("ФИО: " + employee.getFullname() + ", id: " + employee.getId() + ", зп: " + employee.getSalary());
            }
        }
    }

    public void printEmployeesWithSalaryGreaterOrEqualThan(int bound) {
